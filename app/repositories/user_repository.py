...
from app.api.schemas.task import TaskSchemas
from app.api.schemas.comment import CommSchemas
from app.db.models import Task, User, Comment
from app.utils.rep_utils import find_by_id
...
class SQLAlchemyUserRepository(UserRepository):
    ...
    async def get_users(self) -> list[UserFullSchema]:
        query = await self.session.execute(select(User))
        return query.scalars().all()
      
    async def create_user(self, userC: UserBaseSchema) -> UserFullSchema:
    ...
    async def get_user(self, user_id: int) -> UserFullSchema:
        user = await find_by_id(self.session, User, user_id)
        return user
    
    async def update_user(self, user_id: int, userU: UserUpdate) -> UserFullSchema:
        user = await find_by_id(self.session, User, user_id)
    ...
    async def user_task(self, user_id) -> list[TaskSchemas]:
